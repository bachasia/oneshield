<template>
  <AppLayout title="Groups">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        <p class="text-sm text-gray-500">Organize mesh sites into routing groups</p>
      </div>
      <button
        @click="openCreate"
    ...
    <div v-if="groups.length === 0" class="bg-white rounded-xl border border-gray-200 border-dashed py-16 text-center">
      <div class="flex flex-col items-center gap-3 text-gray-400">
        ...
        <div>
          ...
          <p class="text-sm mt-1">Groups let you route payments to specific sets of mesh sites.</p>
        </div>
        <button @click="openCreate" class="mt-2 text-sm text-indigo-600 hover:text-indigo-700 font-medium">Create your first group →</button>
      </div>
    </div>
      ...
      >
        ...
        <div class="flex items-center justify-between pt-3 border-t border-gray-100">
          <div class="flex items-center gap-1.5 text-sm text-gray-600">
            ...
            <span class="font-semibold text-gray-800">{{ group.mesh_sites_count }}</span>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
          ...
          <button @click="closeModal" class="ml-auto text-gray-400 hover:text-gray-600">
        ...
        <form @submit.prevent="submitGroup" class="px-6 py-5 space-y-4">
          ...
          <div class="flex gap-3 pt-1">
            <button type="button" @click="closeModal" class="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50 font-medium transition-colors">
              Cancel
            </button>
            <button type="submit" :disabled="groupForm.processing" class="flex-1 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white px-4 py-2.5 rounded-lg text-sm font-medium transition-colors">
