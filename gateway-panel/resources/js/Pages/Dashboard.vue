<template>
  <AppLayout title="Dashboard">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <StatCard label="Total Sites"         :value="stats.total_sites" />
      <StatCard label="Active Sites"        :value="stats.active_sites" />
      <StatCard label="Transactions Today"  :value="stats.today_transactions" />
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-for="tx in recent_transactions" :key="tx.id" class="hover:bg-gray-50">
              <td class="py-3 font-mono text-xs text-gray-600">{{ tx.order_id }}</td>
              <td class="py-3 font-medium text-gray-900">{{ tx.currency }} {{ Number(tx.amount).toFixed(2) }}</td>
              <td class="py-3 capitalize text-gray-700">{{ tx.gateway }}</td>
              <td class="py-3">
                <StatusBadge :status="tx.status" />
              </td>
              <td class="py-3 text-gray-600">{{ tx.site?.name }}</td>
              <td class="py-3 text-gray-400 text-xs">{{ formatDate(tx.created_at) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
